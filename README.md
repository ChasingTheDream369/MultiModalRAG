...
The current approach —  grouping with  splitting — works well for structured documents with clear headings. But it has one real weakness: **cross-section relevance**. If an answer requires combining context from two different sections (say, a number defined in "Definitions" and used in "Revenue Analysis"), they land in separate chunks and may not both get retrieved for a query that only matches one of them.

There are several ways to address this, each with different tradeoffs.

---

**Sliding window chunking**

Instead of respecting document structure, you chunk by a fixed token window with overlap:



The overlap means a sentence that sits at the boundary between two sections appears in both adjacent chunks. If a query matches the tail of one section and the head of the next, at least one chunk will contain both sides of that boundary.

The tradeoff: you lose section coherence. A chunk might contain the end of one topic and the start of a completely unrelated one, which confuses the embedder. Works better on prose-heavy documents with few hard section breaks, worse on structured financial reports.

---

**Sentence-window retrieval**

Embed at the sentence level but retrieve at the paragraph level. Each sentence gets its own embedding for high-precision matching, but when a sentence is retrieved you expand the context window to ±N surrounding sentences before sending to the LLM.



This is useful when your documents have long dense paragraphs — you get fine-grained retrieval precision without the LLM receiving an isolated sentence stripped of its surrounding explanation.

---

**Document summary + chunk hybrid (parent-child)**

Build two indexes: one with full-section summaries (parent), one with fine-grained chunks (child). Query hits the child index for precision, but the retrieved chunks are replaced by their parent summary before being sent to the LLM.

This directly solves cross-section relevance for high-level questions — if the query is broad, the parent summary contains the cross-section synthesis already. For specific numeric queries, the child chunk is precise enough on its own. LlamaIndex calls this .

---

**Semantic chunking**

Instead of splitting by token count or title boundaries, compute embeddings for every sentence and split when the cosine similarity between adjacent sentences drops below a threshold — that drop signals a topic change.



This produces chunks that are semantically coherent by construction rather than by structural heuristic. The downside is it is slower at ingest time and sensitive to the threshold — too high and you get tiny fragments, too low and you get chunks spanning unrelated topics.
...
|---|---|---|
|  +  (current) | Structured docs with clear headings | Cross-section relevance |
| Sliding window | Prose-heavy docs, few hard boundaries | Loses section coherence |
| Sentence-window | Long dense paragraphs | More complex retrieval logic |
| Parent-child | Broad + specific queries mixed | Two indexes to maintain |
| Semantic chunking | Topic-coherent chunks without structure | Slow ingest, threshold tuning |

For most financial document corpora the current approach is the right starting point. If you see retrieval failures where the answer spans two sections, parent-child retrieval is the cleanest upgrade — it keeps the precision of fine-grained chunks while giving the LLM broader context when needed.
